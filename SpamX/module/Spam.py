...
async def delayspam(xspam: Client, e: Message): 
    ...
    if len(Rizoelop) == 2:
       ...
       if re.search(Owners.lower(), msg.lower()):
            return await e.reply_text("**Sorry !!** I can't Spam On @RiZoeLX's owner")
...
async def uraid(xspam: Client, e: Message):
      ...
      if Rizoel:
          if int(e.chat.id) in GROUP:
               return await e.reply_text("**Sorry !! i Can't Spam Here.**")
          ok = await xspam.get_users(Rizoel)
          id = ok.id
...
async def stop_all(_, e: Message):
       global unlimited
       unlimited = False
